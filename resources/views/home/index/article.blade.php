...
@section('css')
    ...
    {{--  overtrue 通用分享 css  --}}
    <link rel="stylesheet" href="{{ asset('statics/sharejs/css/share.min.css') }}">

    <style>
        .js-content h1, h2, h3, h4, h5, h6{
            margin-top: 10px;
            margin-bottom: 10px;
            font-weight: bold;
        }
        ...
        .js-content table > tbody > tr:nth-of-type(odd) {
        ...
        <div class="row b-article">
            ...
            <div class="col-xs-12 col-md-12 col-lg-12">
                <ul class="row b-metadata">
                    <li class="col-xs-5 col-md-2 col-lg-2"><i class="fa fa-user"></i> {{ $data->author }}</li>
                    <li class="col-xs-7 col-md-3 col-lg-3"><i class="fa fa-calendar"></i> {{ date('Y-m-d H:i', strtotime($data->created_at)) }}</li>
                    <li class="col-xs-5 col-md-2 col-lg-2"><i class="fa fa-list-alt"></i> <a href="{{ url('blog/category', [$data->category_id]) }}">{{ $data->category_name }}</a>
                    <li class="col-xs-5 col-md-2 col-lg-2"><i class="fa fa-eye"></i> {{ $data->click }}</a>
                    <li class="col-xs-5 col-md-2 col-lg-3 "><i class="fa fa-tags"></i>
                        @foreach($data->tag as $v)
                            <a class="b-tag-name" href="{{ url('blog/tag', [$v->tag_id]) }}">{{ $v->name }}</a>
                        @endforeach
                    </li>
                </ul>
            </div>
            <div class="col-xs-12 col-md-12 col-lg-12 b-content-word">
                <div class="js-content markdown-content">{!! $data->html !!}</div>
                {{--  over 通用分享  --}}
                <p class="b-h-20"></p>
                <div class="social-share"><span style="color: #0B94C1;padding-right: 10px;">点击分享本文:</span></div>
                ...
                <ul class="b-prev-next">
                    <li class="b-prev">
                        ...
                        @if(is_null($prev))
                            <span>没有了</span>
                        @else
                            <a href="{{ url('blog/article', [$prev->id]) }}">{{ $prev->title }}</a>
                        @endif
                    ...
                    <li class="b-next">
                        ...
                        @if(is_null($next))
                            <span>没有了</span>
                        @else
                            <a href="{{ url('blog/article', [$next->id]) }}">{{ $next->title }}</a>
                        @endif
                    </li>
                </ul>
            </div>
...
@section('js')
    ...
    <script>
        ...
        // 定义评论url
        ajaxCommentUrl = "{{ url('blog/comment') }}";
        checkLogin = "{{ url('blog/checkLogin') }}";
        titleName = '{{ $config['WEB_NAME'] }}';
    </script>
    <script src="{{ asset('statics/layer-2.4/layer.js') }}"></script>
    <script src="{{ asset('js/home/comment.js') }}"></script>
    {{--  overtrue 通用分享 js  --}}
    <script src="{{ asset('statics/sharejs/js/social-share.min.js') }}"></script>
@endsection
