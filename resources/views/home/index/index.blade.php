        ...
        @foreach($article as $k => $v)
            <div class="row b-one-article">
                <h3 class="col-xs-12 col-md-12 col-lg-12">
                    <a class="b-oa-title" href="{{ url('blog/article', [$v->id]) }}" target="_blank">{{ $v->title }}</a>
                </h3>
                <div class="col-xs-12 col-md-12 col-lg-12 b-date">
                    <ul class="row">
                        ...
                        <li class="col-xs-7 col-md-3 col-lg-3">
                            <i class="fa fa-calendar"></i> {{ date('Y-m-d H:i', strtotime($v->created_at)) }}
                        </li>
                        <li class="col-xs-5 col-md-2 col-lg-2">
                            <i class="fa fa-list-alt"></i> <a href="{{ url('blog/category', [$v->category_id]) }}" target="_blank">{{ $v->category_name }}</a>
                        </li>
                        <li class="col-xs-5 col-md-2 col-lg-2">
                            <i class="fa fa-eye"></i> {{ $v->click }}</a>
                        </li>
                        <li class="col-xs-5 col-md-2 col-lg-3 "><i class="fa fa-tags"></i>
                            @foreach($v->tag as $n)
                                <a class="b-tag-name" href="{{ url('blog/tag', [$n->tag_id]) }}" target="_blank">{{ $n->name }}</a>
                            @endforeach
                        </li>
                    </ul>
                </div>
                <div class="col-xs-12 col-md-12 col-lg-12">
                    <div class="row">
                        ...
                        <!-- 文章描述开始 -->
                        <div class="col-xs-12 col-sm-6  col-md-6 col-lg-8 b-des-read" style="text-indent: 2em;">
                            {{ $v->description }}
                        </div>
                        <!-- 文章描述结束 -->
                    </div>
                </div>
                <a class=" b-readall" href="{{ url('blog/article', [$v->id]) }}" target="_blank">阅读全文</a>
            </div>
        @endforeach
        <!-- 循环文章列表结束 -->
